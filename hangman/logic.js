...
let randNum = getRandomInt(0,words.length);
let secret = words[randNum];
let pic0 = ".\\hangmanPics\\hangmanEmpty.png";
let pic1 = ".\\hangmanPics\\hangman1wrong.png";
let pic2 = ".\\hangmanPics\\hangman2wrong.png";
let pic3 = ".\\hangmanPics\\hangman3wrong.png";
let pic4 = ".\\hangmanPics\\hangman4wrong.png";
let pic5 = ".\\hangmanPics\\hangman5wrong.png";
let pic6 = ".\\hangmanPics\\hangman6wrong.png";
let pic7 =".\\hangmanPics\\hangmanWin.png";
let pictures = [pic0,pic1,pic2,pic3,pic4,pic5,pic6];
let wrong = 0;
document.getElementById("person").src = pictures[wrong];
let newWord = [];
let newHidden = "";
let guessCount=1;
let guessHistory= [];

let hiddenWord =''; //change this
for (letter in secret){
    hiddenWord += "-";
}
document.getElementById("hidden word").innerText = hiddenWord;

function NewGame(){//reset guessed letters
    randNum = getRandomInt(0,words.length);
    secret = words[randNum];
    wrong=0;
    document.getElementById("person").src = pictures[wrong];
    hiddenWord ='';
    for (letter in secret){
...
function wrongGuess(){
    wrong++;
    console.log(wrong,"wrong guesses");
    document.getElementById("person").src = pictures[wrong];
}

function getRandomInt(min, max){ // generates a random number
	min = Math.ceil(min);
	max = Math.floor(max);
...
function CheckIn(){
    ...
    if ((guess.charCodeAt()>64&&guess.charCodeAt()<91)||(guess.charCodeAt()>96&&guess.charCodeAt()<123)){
        //console.log("letter", guess);
        let lGuess = guess.toLowerCase();
        //console.log(lGuess);
        checkGuess(lGuess);
    }
    else{
...
function checkGuess(guess){
    ...
    if(secret.toLocaleLowerCase().indexOf(guess)==-1){
            wrongGuess();
        }
    for(letter in secret){
        //console.log("letter:",secret[letter].toLowerCase(),"guessed:",guess);
        if((guess==secret[letter].toLowerCase())||(secret[letter].toLowerCase()==newWord[letter])){
            if(secret[letter].toLowerCase()==newWord[letter]){
                // console.log("match",guess);
                // console.log("before",newWord);
                newWord[letter] =secret[letter].toLowerCase();
                // console.log("after",newWord);
            }
            else{
                // console.log("match",guess);
                // console.log("before",newWord);
                newWord[letter] =guess;
                //console.log("after",newWord);
            }
        }
        else{
    ...
    for(item in newWord){
        newHidden += newWord[item];
    }
    turnUpdate();
}

function turnUpdate(){
    newHidden = newHidden.slice(newHidden.length - secret.length)
    document.getElementById("hidden word").innerHTML = newHidden;
    document.getElementById("guessHistory").innerHTML = guessHistory;
    guessCount++;
    checkWinLose()
}
...
function checkWinLose(){
    let winCount = 0;
    for(item in newHidden){
        //console.log(newHidden[item], "ascii",newHidden[item].charCodeAt())
        if((newHidden[item].charCodeAt()>96&&newHidden[item].charCodeAt()<123)){
            winCount++;
        }
    }
    //console.log(winCount)
    if (winCount == secret.length){
        document.getElementById("result").innerHTML="Win";
        document.getElementById("person").src = pic7;
        end();
        // document.write("win") //works as intended but ends game abruptly
    }
    if (wrong==6){
        // document.write("lose") //works as intended but ends game abruptly
...
function end(){
    ...
    document.getElementById("enddingText").style.visibility = "visible";
    document.getElementById("enddingButton").style.visibility = "visible";
    document.getElementById("word").innerHTML = secret;
    document.getElementById("enddingWord").style.visibility = "visible";
...
function Reset(){
    ...
    document.getElementById("enddingText").style.visibility = "hidden";
    document.getElementById("enddingButton").style.visibility = "hidden";
    document.getElementById("enddingWord").style.visibility = "hidden";
    NewGame();
}
